...
import { NotificationItem, addNotification, NotificationState, updateNotificationState } from 'src/navigation/notifications/notificationsSlice'
...
import { IdentityDid, SignerEd25519, Rule, SignerDid } from '@dedis/cothority/darc';
...
export class ConsentService {
  ...
  constructor(exchangeService: ExchangeService, messageSender: MessageSender, consentRepository: Repository<ConsentRecord>, connectionRepository: Repository<ConnectionRecord>, wallet: Wallet) {
  ...
  async requestConsentInformation(invitationId: string) {
    ...
    const connectionRequest = await this.exchangeService.acceptInvitation(plainToClass(ConnectionInvitationMessage, record.invite!))
    ...
    const requestMessage = new ConsentInformationRequestMessage({ cohortID: record.cohortID })
  ...
  async receiveConsentInformation(messageContext: InboundMessageContext<ConsentInformationResponseMessage>) {
  ...
  async grantConsent(invitationId: string) {
    ...
    const latestBlock = await this.getLatestBlock(skipchainRpc, genesisBlock, false);
    ...
    const signer = new SignerDid(async (data: Buffer, did: string) => { return await this.wallet.sign(data, verkey); }, { did: publicDid, method: 'sov' })
  ...
  private async getLatestBlock(tthis: SkipchainRPC, latestID: Buffer, verify: boolean = true) {
