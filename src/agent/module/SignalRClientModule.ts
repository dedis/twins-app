...
export class SignalRClientModule {
    ...
    constructor(provisioningService: ProvisioningService, connectionService: ConnectionService, envelopeService: EnvelopeService, messageReceiver: MessageReceiver, agentConfig: AgentConfig) {
    ...
    registerHandlers() {
        this.connection?.on('Authorize', async (nonce: string) => {
            ...
            const connection = await this.connectionService.find(provisioningRecord?.agencyConnectionId);
            const outboundMessage = createOutboundMessage(connection!, response);
            const packedMessage = await this.envelopeService.packMessage(outboundMessage);
