...
export const HomeScreen = ({navigation}) => {
  ...
  const [provisionState, setAgentState] = React.useState<AgentState>(
    AgentState.INIT,
  );
      ...
      case 'android':
        if (!exists) {
          ...
          if (granted === PermissionsAndroid.RESULTS.GRANTED) {
            ...
            await (agentModule.getAgent() as EdgeAgent).ledger.connect(
              'buildernet',
              {genesis_txn: path},
            );
          } else {
  ...
  useEffect(() => {
    ...
    const inflateStateFromDB = async () => {
      ...
      const items = connections
        .filter((connection) => connection.state === ConnectionState.COMPLETE)
        .map((connection) => ({
  ...
  const onShareSecret = () => {
    navigation.navigate('SecretShare', {
      key: agentConfig.walletCredentials.key,
    });
  };
      ...
      return (
        <SafeAreaView style={[styles.safeArea]}>
          <Layout
            ...
            <Text>
              We found an existing wallet but the key doesn't exist in your
              keychain.
            </Text>
          </Layout>
          <Input
          ...
          <View
            ...
            <Button onPress={() => onCreateNewWallet(true)}>
              Create New Wallet
            </Button>
        ...
        <SafeAreaView style={[styles.safeArea]}>
          <Layout
            ...
            <Input
              ...
              onChangeText={(nextValue) => setDidSeed(nextValue)}
            />
            <Button onPress={() => onCreateNewWallet(false)}>
              Create New Wallet
            </Button>
          </Layout>
        </SafeAreaView>
