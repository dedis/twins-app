import { Button, Icon, IconElement, Layout, Select, SelectOption, SelectOptionType, StyleService, Text, TopNavigation, TopNavigationAction, useStyleSheet } from '@ui-kitten/components';
import React, { useState } from 'react';
import { ImageStyle, SafeAreaView } from 'react-native';

export const RecoverSecretScreen = ({ navigation }) => {
  const maxTotalShares = 10;
  const thresholdSharesOptions = Array.from({ length: maxTotalShares }, (_, i) => i+1).map(x => ({ text: `${x} `}));
  ...
  const onScanShares = () => {
    navigation.navigate('ScanShares', { totalShares: Number.parseInt((thresholdShares as SelectOptionType).text)})
  }
