import { Button, Card, Layout, Spinner, StyleService, Text, TopNavigation, useStyleSheet } from '@ui-kitten/components';
...
export const CredentialScreen = ({ navigation }) => {
    ...
    // @ts-ignore
    const offer: CredentialOfferMessage = plainToClass(CredentialOfferMessage, notification.payload.offer);
    // @ts-ignore
    const [ credentialState, setCredentialState ] = useState<CredentialState>(CredentialState.OfferReceived);
    ...
    const sendCredentialRequest = async () => {
        setBusy(true);
        const credentialRecord = await agentModule.getAgent()?.credentials.find(notificationId)!;
            ...
            >
                ...
                <View style={{ margin: 10, flex: 1, justifyContent: 'center', alignContent: 'center' }}>
                    <Card style={{ marginBottom: 25 }}>
                        ...
                        <Button disabled={credentialState !== CredentialState.OfferReceived} onPress={sendCredentialRequest}>
                            { credentialState === CredentialState.OfferReceived ? 'Accept' : 'Accepted' }
                        </Button>
                    </Card>
                </View>
            </Layout>
